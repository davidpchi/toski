...
export const getCommandersByPlayerName = (
	...
	const commanders = Object.values(
		playedCommanderDictionary(state.stats.matches, playerName),
	);
 ...
 */
function playedCommanderDictionary(
	matches: Match[],
	playerId: string,
): { [id: string]: Commander } {
			...
			if (player.name === playerId) {
				...
				for (const currentCommanderName of currentCommanderNames) {
					...
					if (potentialCommanderObj === undefined) {
						...
						playedCommanderDictionary[potentialCommanderObj.id].matches.push(
							currentMatch.id,
						);
						if (player.rank === "1") {
